...
const Tvpage = () => {
  ...
  useEffect(() => {
    (async () => {
      const genreResponse = await axios.get(urls.genres_tv);
      setGenres(genreResponse.data.genres);

      const moviesResponse = await axios.get(
        `${
          urls.discover_tv.src + (selected !== 0 ? selected : "")
        }&page=${page}`
      );
      setMovies((m) =>
        m.concat(
      ...
      <div className="flex flex-col overflow-hidden md:flex-row">
        <div className="flex flex-row bg-black text-white py-4 px-6 overflow-x-scroll scrollbar-hidden md:flex-col">
          ...
          <span
            ...
            className={`my-3 mx-2 whitespace-no-wrap cursor-pointer text-white ${
              selected === 0 ? "text-opacity-100" : "text-opacity-50"
            }`}
          >
            All
          </span>
          {genres.map((genre: GenreModel) => (
            <span
              ...
              className={`my-3 mx-2 whitespace-no-wrap cursor-pointer text-white  ${
                selected === genre.id ? "text-opacity-100" : "text-opacity-50"
              }`}
            >
              {genre.name}
            </span>
